...
{	u16 Intr;
	...
	{	Intr=InterruptOccured & IRQMask;
		if(Intr) break;
		msleep(1); } /* XXX - blocking delay - fix */
...
{	u16 Config, PhysCyls, Res2, PhysHeads, UnfBytesPerTrack;
	...
	u8 SerialNum[20];
	u16 BufType, BufSize, ECCBytes;
	u8 FirmwareRev[8], Model[40], MaxMult, Vendor3;
	u16 DwordIO;
	u8 Vendor4, Capability;
	u16 Res50;
	u8 Vendor5, PIOMode, Vendor6, DMAMode;
	u16 LogValid, LogCyls, LogHeads, LogSects;
	u32 TotalSects;
	u8 MultSect, MultSectValid;
	u32 LBASects;
...
drive will go comatose */
	insw(IOAdr + ATA_REG_DATA, (u16 *)&DriveInfo,
		sizeof(DriveInfo) / 2);
	Temp2=1;
...
/* model name is not byte swapped for NEC, Mitsumi, and Pioneer drives */
	...
	{	printf("%c", DriveInfo.Model[Temp ^ Temp2]);
		...
		printf("\n""                CHS=%u:%u:%u, ",
			DriveInfo.PhysCyls, DriveInfo.PhysHeads,
			DriveInfo.PhysSects);
...
{	unsigned Temp;
	...
	{	if((inb(IOAdr + ATA_REG_STAT) & 0x80) == 0) break;
		msleep(1); }	/* this _must_ be polled, I guess (sigh) */
	return(Temp == 0); }
/*****************************************************************************
	name:	ataProbe
*****************************************************************************/
...
/* poke at the interface to see if anything's there */
		DEBUG(printf("ataProbe: poking interface 0x%03X\n", IOAdr);)
...
/* soft reset both drives on this I/F (selects master) */
		DEBUG(printf("ataProbe: found something on I/F 0x%03X, "
			"doing soft reset...\n", IOAdr);)
...
/* wait for master */
		...
		if(Temp == 0)
		{	DEBUG(printf("ataProbe: no master on I/F 0x%03X\n",
				IOAdr);)
			goto NO_DRIVES; }
/* identify master */
...
/* no response from slave: re-select master and continue */
		{	ataSelect(IOAdr, 0xA0);
			...
			DEBUG(printf("ataProbe: no slave on I/F 0x%03X\n",
				IOAdr);)
			continue; }
/* identify slave */
...
sector count) to < 256 */
		{	Temp=(Cmd->Count + ATA_SECTSIZE - 1)
			...
			DEBUG(printf("ataCmd: ready to read %lu "
				"sector(s) of %lu\n", Count, Temp);)
...
(the sector count) to < 256 */
		{	Temp=(Cmd->Count + ATA_SECTSIZE - 1)
				>> ATA_LG_SECTSIZE;
			Count=min(Temp, Drive[Cmd->Dev].MultSect);
			DEBUG(printf("ataCmd: ready to write %lu "
				"sector(s) of %lu\n", Count, Temp);)
/* compute CHS or LBA register values and write them, along with CmdByte */
			ataCmd2(Cmd, Count, CmdByte);
/* await DRQ */
...
{	u8 Res0, Min, Sec, Frame; } atapimsf;

typedef struct			/* 4 bytes */
{	u16 TocLen;
	u8 FirstTrk;
	u8 LastTrk; } atapitocheader;
...
to great confusion. The size of this structure _IS_ 8 bytes. */
	...
	union
	{	u32 Block;
		atapimsf Time; } Where; } atapitocentry;

typedef struct
{	atapitocheader Hdr;
	atapitocentry Ent[100]; } atapitoc;
/*****************************************************************************
	name:	atapiCmd2
...
{	u16 IOAdr, Got;
	...
	{	printf("atapiCmd2: could not select drive\n");
		return(-1); }
	DEBUG(printf("atapiCmd2: writing register file\n");)
	...
	{	if((inb(IOAdr + ATA_REG_STAT) & 0x88) == 0x08) break;
		msleep(1); }	/* this _must_ be polled, I guess */
	if(Temp == 0)
...
/* read and discard surplus data */
		for(Got -= Temp; Got > 1; Got -= 2) inw(IOAdr + ATA_REG_DATA);
		if(Got) inb(IOAdr + ATA_REG_DATA);
/* advance pointers */
		Cmd->Data += Temp;
		Cmd->Count -= Temp;
...
/* convert general block device command code into ATAPI packet commands */
	if(Cmd->Cmd == DRV_CMD_RD) Pkt[0]=ATAPI_CMD_READ10;
	else
...
/* read just the header at first */
	...
	DEBUG(printf("atapiTOC: calling atapiTOCEnt with Count=%u\n", Temp);)
...
/* lock memory (disable paging) */
	...
	_go32_dpmi_get_protected_mode_interrupt_vector(IRQ14_VECTOR,
		&OldIRQ14Vector);
	_go32_dpmi_get_protected_mode_interrupt_vector(IRQ15_VECTOR,
		&OldIRQ15Vector);

	NewVector.pm_selector=_my_cs();
	NewVector.pm_offset=irq14;		/* ptr -> int */
	_go32_dpmi_allocate_iret_wrapper(&NewVector);
#if defined(CHAIN_INTS)
	_go32_dpmi_chain_protected_mode_interrupt_vector(IRQ14_VECTOR,
		&NewVector);
#endif
	_go32_dpmi_set_protected_mode_interrupt_vector(IRQ14_VECTOR,
		&NewVector);

	NewVector.pm_selector=_my_cs();
	NewVector.pm_offset=irq15;		/* ptr -> int */
	_go32_dpmi_allocate_iret_wrapper(&NewVector);
#if defined(CHAIN_INTS)
	_go32_dpmi_chain_protected_mode_interrupt_vector(IRQ15_VECTOR,
		&NewVector);
#endif
	_go32_dpmi_set_protected_mode_interrupt_vector(IRQ15_VECTOR,
		&NewVector); }
/*****************************************************************************
	name:	irqEnd
...
/* play from 13:11:32 to 18:01:00 -- Zeppelin */
	{	Temp=atapiPlay(WhichDrive, (13ul * 60 + 11) * 75 + 32,
			(18ul * 60 + 1) * 75 + 0);
		if(Temp) printf("atapiPlay returned error %d, "
			"retrying...\n", Temp);
		else break; }
#endif
